function [isGoal,openNode,ii] = move5(currentNode,goalPosition,openNode,closedNode,ii)
currentPosition = currentNode(:,1:3);
isGoal = 0;
threshold  =  0.2;
[X,Y,theta] = calculateNewPose(currentPosition(:,1),currentPosition(:,2),currentPosition(:,3),0,10);
...
if(inCSpace(newPosition(:,1:2)) && outObstacle(newPosition(:,1:2)))
   ...
   if((isempty(isOpenmemberOutput) || any(isOpenmemberOutput)~=1)&&(isempty(isClosedmemberOutput) || any(isClosedmemberOutput)~=1))
         openNode(size(openNode,1)+1,:)= [newPosition(:,1) newPosition(:,2) newPosition(:,3) calculatedF calculatedG calculatedH currentNode(:,8) ii+1 0 10];
         ii=ii+1;
